...
from utils.chrome_keyboard import close_page, back_page, refresh_page, pre_page, wait_page
...
class JS(QDialog):
    def __init__(self, parent=None):
        ...
        btn1.clicked.connect(lambda x: continue_to_page()
                                       ...
                                       or close_page()
                                       or back_left_screen())
        btn5 = QPushButton("无刷新视频页下载并关闭", self)
        btn5.clicked.connect(lambda x: download_from_video_page()
                                       or pre_page()
                                       or close_page()
                                       or back_left_screen())
